...
$drivers = @(
  ...
  @{ Name = "Intel Management Engine";
    driverID = "$(pnputil /enum-devices /problem | Select-string 'VEN_8086&DEV_8D3D&SUBSYS_212A103C')";
...
foreach ($driver in $drivers) {
  if ( [bool]$driver.driverID -or ($driver.Name -eq "Intel Rapid Storage Technology" -and !(Test-Path -Path $intelRSTExe))) {
