...
function ScheduleTaskForNextBoot () {
  ...
  $ActionScript = "C:\Windows\Setup\Scripts\set_lockscreen_wallpaper.ps1 -scheduledTaskName $scheduledTaskName"

  $Action = New-ScheduledTaskAction -Execute "PowerShell" -Argument "-WindowStyle Hidden -ExecutionPolicy Bypass -NoProfile -File `"$ActionScript`""
